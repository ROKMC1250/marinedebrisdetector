...
class SegmentationHead(nn.Module):
    def __init__(self, in_channels, num_classes, kernel_size=3, activation=None, upsampling=1):
        ...
        self.segmentationhead = nn.Sequential(
            nn.Conv2d(in_channels, out_channels = num_classes, kernel_size=kernel_size, padding=kernel_size // 2),
...
class FullNet(nn.Module):
    def __init__(self, num_classes, channels=[32, 64, 96, 128], layers=[2, 3, 6, 3], num_subnet=5, kernel_size = 3, drop_path = 0.0, save_memory=True, inter_supv=True, head_init_scale=None) -> None:
        super().__init__()
        ...
        self.stem = nn.Sequential(
            nn.Conv2d(12, channels[0], kernel_size=4, stride=4),
        ...
        if not inter_supv:
            ...
            self.seg = SegmentationHead(in_channels=channels[-1], num_classes=num_classes)
        else:
            self.cls_blocks = nn.ModuleList([Classifier(in_channels=channels[-1], num_classes=num_classes) for _ in range(4) ])
            self.seg_blocks = nn.ModuleList([SegmentationHead(in_channels=channels[-1], num_classes=num_classes) for _ in range(4) ])
            if num_classes<=1000:
                channels.reverse()
        ...
        for i in range(self.num_subnet):
            ...
            if (i+1) % interval == 0:
                ...
                if i != self.num_subnet-1:
                    a = self.decoder_blocks[i//interval](c3)
                    x_img_out.append(a)
